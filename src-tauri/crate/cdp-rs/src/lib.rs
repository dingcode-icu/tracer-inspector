...
impl CdpClient {
    ...
    /// Creates a `CdpConnection` to a addr.
    /// 
    /// # Examples
    /// 
    /// ```
    ...
    /// let cdp = client.connect_to_addr("localhost", 6086);
    /// }
    /// ```
    pub fn connect_to_addr(&mut self, host: String, port: u16) -> Result<CdpConnection, ClientError> {
        self.host = host;
        self.port = port;
...
impl CdpConnection {
    ...
    pub fn send(&mut self, method: &'static str, parms: Vec<(&'static str, MessageParameter)>) -> Result<Value, MessageError> {
        ...
        println!("send message:{:?}", data.to_string());
    ...
    pub fn wait_message(&mut self) -> Result<Value, MessageError> {
        if let Ok(msg) = self.socket.read_message() {
            ...
            let val: Result<Value, serde_json::Error> = serde_json::from_str(&text);
